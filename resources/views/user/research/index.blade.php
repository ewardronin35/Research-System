    ...
    <button type="button" class="btn btn-danger btn-sm delete-btn" title="Delete"
        ...
        @endforeach
                        ...
                        <div class="tab-pane fade" id="add-research" role="tabpanel" aria-labelledby="add-research-tab">
                            <div class="card">
                                <div class="card-body">
                                    <form id="researchForm" action="{{ route('user.research.store') }}" method="POST" enctype="multipart/form-data" class="needs-validation" novalidate>
                                        @csrf
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        FilePond.setOptions({
            ...
            maxFileSize: '10MB',
            server: {
                url: '{{ route("user.research.filepond-upload") }}', // Create this route
                headers: {
        ...
        // Create a FilePond instance
         FilePond.create(document.querySelector('#research_file'), {
            server: {
                url: '{{ route("user.research.filepond-upload") }}',
            ...
            editPond = FilePond.create(editPondInput, {
                ...
                server: {
                    ...
                    remove: {
                        ...
                        url: '{{ route("user.filepond.remove") }}',
                        method: 'DELETE',
                        headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' }
                    },
                    load: {
...
$(document).on('click', '.edit-btn', function() {
            ...
            $.ajax({
                url: `/user/api/research/${id}`, // <-- UPDATED URL
                type: 'GET',
                dataType: 'json',
                success: function(data) {
                    ...
                    var updateUrl = '{{ url("user/research") }}/' + data.id;
